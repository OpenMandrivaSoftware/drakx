...
covers the entire OpenMandriva Lx distribution. If you agree with all the
...
sub chooseCd() {
    N("The OpenMandriva Lx installation is distributed on several CD-ROMs. If a
...
There are thousands of packages available for OpenMandriva Lx, and to make it
simpler to manage, they have been placed into groups of similar
applications.

OpenMandriva Lx sorts package groups in four categories. You can mix and
...
installed. By default OpenMandriva Lx will automatically start any installed
services at boot time. Even if they are safe and have no known issues at
the time the distribution was shipped, it is entirely possible that
security holes were discovered after this version of OpenMandriva Lx was
...
WindowMaker, etc.) bundled with OpenMandriva Lx rely upon.
...
sub doPartitionDisks() {
    N("You now need to decide where you want to install the OpenMandriva Lx
...
recommended if you want to use both OpenMandriva Lx and Microsoft Windows on
the same computer.
...
your hard drive and replace them with your new OpenMandriva Lx system, choose
...
OpenMandriva Lx operating system installation.

Click on \"%s\" if you wish to select partitions which will be checked for
bad blocks on the disk.", N("Next"), N("Previous"), N("Advanced"));
}
sub installUpdates() {
    N("By the time you install OpenMandriva Lx, it's likely that some packages will
...
to change it later with the draksec tool, which is part of OpenMandriva Lx
Control Center.
...
installation of your OpenMandriva Lx system. If partitions have already been
...
OpenMandriva Lx operating system.
...
upgrade of an existing OpenMandriva Lx system:
...
currently installed on your OpenMandriva Lx system. Your current partitioning
scheme and user data will not be altered. Most of the other configuration
steps remain available and are similar to a standard installation.

Using the ``Upgrade'' option should work fine on OpenMandriva Lx systems
running version \"8.1\" or later. Performing an upgrade on versions prior
to OpenMandriva Lx version \"8.1\" is not recommended.", N("Install"), N("Upgrade"));
}
sub selectKeyboard() {
...
still under development. For that reason, OpenMandriva Lx's use of UTF-8 will
...
operating systems may offer you one, but OpenMandriva Lx offers two. Each of
...
system you may change it by running PrinterDrake from the OpenMandriva Lx
...
OpenMandriva Lx Control Center after the installation has finished to benefit
...
OpenMandriva Lx partition. Be careful, all data on this drive will be lost
